...
B_SHAPE='square'    # attack 后门触发器的形状（这里是 "square"，即正方形触发器）。
B_LABEL=0 #后门攻击的目标类别标签（这里是 0）。
LCONST1=(0.5)
LCONST2=(0.5) #超参数常数，用于控制损失函数中不同部分的权重。
#遍历 LCONST1 的所有值（这里只有 0.5）。
#通过改变 LCONST1，观察该常数对实验的影响。
