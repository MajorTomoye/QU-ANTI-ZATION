...
def train_w_backdoor( \
    ...
    f32_closs = 0.
    f32_bloss = 0.
    q08_closs = {} #记录量化后干净数据的损失
    q08_bloss = {} #记录量化后后门数据的损失
    ...
    for cdata, ctarget, bdata, btarget in tqdm(dataloader, desc='[{}]'.format(epoch), total=num_iters):
        ...
        fcloss, fbloss = taskloss(coutput, ctarget), taskloss(boutput, ctarget) #量化前干净数据的平均损失，后门数据的平均损失
        tloss = fcloss + const2 * fbloss #总损失
        ...
        for eachbit in nbits:
            with QuantizationEnabler(net, wqmode, aqmode, eachbit, silent=True):
                ...
                qcloss = taskloss(qcoutput, ctarget) #量化后干净数据的平均损失
                qbloss = taskloss(qboutput, btarget) #量化后后门数据的平均损失
                tloss += const1 * (qcloss + const2 * qbloss) #总损失补充项
